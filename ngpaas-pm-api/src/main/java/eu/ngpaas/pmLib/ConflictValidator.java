...
public final class ConflictValidator {
    ...
    public static SimpleResponse conflictResolution(PolicyRule npRule, ArrayList<PolicyRule> crl) {
        /* There are two cases that should be taken into account.
            1. npRule has a higher priority compared with all rules in crl --> Put all rules in crl in Pending state and clean flow tables
            2. At least one rule from crl has a higher or equal priority to npRule --> Keep the rules in crl as active and move npRule to Pending
     ...
     * @return a SimpleResponse object with the success/fail message
     */
    public static SimpleResponse newCheckConflict(FlowEntry enforcedEntry, ForwardingObjective newEntry, List<DeviceId> targetDevices) {
        ...
        for (Criterion enforcedCriteria : enforcedEntry.selector().criteria()) {
            for (Criterion newCriteria : newEntry.selector().criteria()) {
                if (enforcedCriteria.type() != newCriteria.type()) continue;
                ...
                switch (enforcedCriteria.type()) {
                    case ETH_TYPE:
                        ...
                        if (enforced_etc.ethType() != new_etc.ethType()) return sr;
                        break;
                    case IP_PROTO:
                        ...
                        if (enforced_ippc.protocol() != new_ippc.protocol()) return sr;
                        break;
                    case IPV4_SRC:
                    case IPV4_DST:
                        IPCriterion enforced_ipc = (IPCriterion) enforcedCriteria;
                        IPCriterion new_ipc = (IPCriterion) newCriteria;
                        if (!checkCidrInCidr(new_ipc.ip().getIp4Prefix(), enforced_ipc.ip().getIp4Prefix())) return sr;
                        break;
                    case ETH_SRC:
                    case ETH_DST:
                        enforced_ethc = (EthCriterion) enforcedCriteria;
                        new_ethc = (EthCriterion) newCriteria;
                        if (!Long.valueOf(enforced_ethc.mac().toLong()).equals(new_ethc.mac().toLong())) return sr;
                        break;
                    case ETH_SRC_MASKED:
                    case ETH_DST_MASKED:
                        enforced_ethc = (EthCriterion) enforcedCriteria;
                        new_ethc = (EthCriterion) newCriteria;
                        if (!checkMacInMac(new_ethc.mac(), new_ethc.mask(), enforced_ethc.mac(), enforced_ethc.mask()))
                            return sr;
                        break;
                    case TCP_SRC:
                    case TCP_DST:
                        ...
                        if (enforced_tcppc.tcpPort() != new_tcppc.tcpPort()) return sr;
                        break;
                    case TCP_SRC_MASKED:
                    case TCP_DST_MASKED:
                        enforced_tcppc = (TcpPortCriterion) enforcedCriteria;
                        new_tcppc = (TcpPortCriterion) newCriteria;
                        if (!checkPortInPort(new_tcppc.tcpPort(), new_tcppc.mask(), enforced_tcppc.tcpPort(), enforced_tcppc.mask()))
                            return sr;
                        break;
                    case UDP_SRC:
                    case UDP_DST:
                        ...
                        if (enforced_udppc.udpPort() != new_udppc.udpPort()) return sr;
                        break;
                    case UDP_SRC_MASKED:
                    case UDP_DST_MASKED:
                        enforced_udppc = (UdpPortCriterion) enforcedCriteria;
                        new_udppc = (UdpPortCriterion) newCriteria;
                        if (!checkPortInPort(new_udppc.udpPort(), new_udppc.mask(), enforced_udppc.udpPort(), enforced_udppc.mask()))
                            return sr;
                        break;
                }
        ...
        //If we arrive here we know for sure that the flow entries are not independent. If they perform different actions, that will be a conflict.
        for (Instruction newInstruction : newEntry.treatment().allInstructions()) {
            ...
            for (Instruction enforcedInstruction : enforcedEntry.treatment().allInstructions()) {
                if (enforcedInstruction.type() != newInstruction.type()) continue;
                switch (enforcedInstruction.type()) {
                    case NOACTION: //DROP
                        matched = true;
                        break;
                    case OUTPUT:
                        Instructions.OutputInstruction enforced_output = (Instructions.OutputInstruction) enforcedInstruction;
                        Instructions.OutputInstruction new_output = (Instructions.OutputInstruction) newInstruction;
                        if (!enforced_output.port().equals(new_output.port())) return sr2;
                        matched = true;
                        break;
                    case L2MODIFICATION:
                        ...
                        if (enforced_l2mod.subtype() != new_l2mod.subtype()) continue;
                        switch (enforced_l2mod.subtype()) {
                            case ETH_SRC:
                            case ETH_DST:
                                L2ModificationInstruction.ModEtherInstruction enforced_l2modethi = (L2ModificationInstruction.ModEtherInstruction) enforced_l2mod;
                                L2ModificationInstruction.ModEtherInstruction new_l2modethi = (L2ModificationInstruction.ModEtherInstruction) new_l2mod;
                                if (!Long.valueOf(enforced_l2modethi.mac().toLong()).equals(new_l2modethi.mac().toLong()))
                                    return sr2;
                                matched = true;
                                break;
                        }
                    ...
                    case L3MODIFICATION:
                        ...
                        if (enforced_l3mod.subtype() != new_l3mod.subtype()) continue;
                        switch (enforced_l3mod.subtype()) {
                            case IPV4_SRC:
                            case IPV4_DST:
                                L3ModificationInstruction.ModIPInstruction enforced_l3modethi = (L3ModificationInstruction.ModIPInstruction) enforced_l3mod;
                                L3ModificationInstruction.ModIPInstruction new_l3modethi = (L3ModificationInstruction.ModIPInstruction) new_l3mod;
                                if (!new_l3modethi.ip().equals(enforced_l3modethi.ip())) return sr2;
                                matched = true;
                                break;
                        }
                        break;
                    case L4MODIFICATION:
                        ...
                        if (enforced_l4mod.subtype() != new_l4mod.subtype()) continue;
                        switch (enforced_l4mod.subtype()) {
                            case TCP_SRC:
                            case TCP_DST:
                            case UDP_SRC:
                            case UDP_DST:
                                L4ModificationInstruction.ModTransportPortInstruction enforced_l4modtpi = (L4ModificationInstruction.ModTransportPortInstruction) enforced_l4mod;
                                L4ModificationInstruction.ModTransportPortInstruction new_l4modtpi = (L4ModificationInstruction.ModTransportPortInstruction) new_l4mod;
                                if (enforced_l4modtpi.port().toInt() != new_l4modtpi.port().toInt()) return sr2;
                                matched = true;
                                break;
                        }
                        break;
    ...
    private static Boolean allConditionsIndependent(CopyOnWriteArrayList<CopyOnWriteArrayList<PolicyCondition>> apcs,
        ...
        for (CopyOnWriteArrayList<PolicyCondition> activeClause : apcs) {
            for (CopyOnWriteArrayList<PolicyCondition> newClause : npcs) {
                ...
                intersection = (ArrayList<String>) CollectionUtils.intersection(activeClauseDict.keySet(), newClauseDict.keySet());
