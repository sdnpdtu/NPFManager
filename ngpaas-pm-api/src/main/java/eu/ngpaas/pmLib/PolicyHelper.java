...
public class PolicyHelper {
        ...
        for (CopyOnWriteArrayList<PolicyCondition> clause : pr.getPolicyConditions() ){
            for (PolicyCondition pc : clause) {
                if (pc.getPolicyVariable().equalsIgnoreCase(variable)) {
                    switch (type) {
                        case IPV4: isValidType = isIPV4address(pc.getPolicyValue()); break;
                        case MAC:  isValidType = isMACaddress(pc.getPolicyValue()); break;
                        case PORT: isValidType = isPort(pc.getPolicyValue()); break;
                        default: return new SimpleResponse("Invalid PolicyVariableType provided", false);
                    }
                    if (!isValidType) 
        ...
        Boolean isValidType;
            for (PolicyAction pa : pr.getPolicyActions()) {
                if (pa.getPolicyVariable().equalsIgnoreCase(variable)) {
                    switch (type) {
                        case IPV4: isValidType = isIPV4address(pa.getPolicyValue()); break;
                        case MAC:  isValidType = isMACaddress(pa.getPolicyValue()); break;
                        case PORT: isValidType = isPort(pa.getPolicyValue()); break;
                        default: return new SimpleResponse("Invalid PolicyVariableType provided", false);
                    }
                    if (!isValidType) return new SimpleResponse("Policy Action value '" + 
                        pa.getPolicyValue() + "' does not have a valid format.", false);
                }
            }
    ...
    public static SimpleResponse validateConditionVariableRelations(PolicyRule pr, 
        HashMap<String, CopyOnWriteArrayList<CopyOnWriteArrayList<String>>> mustCoexistDict,
        HashMap<String, CopyOnWriteArrayList<String>> mustNotCoexistDict) {
            ...
            for (PolicyCondition pc: clause) {
                try {
                    ...
                    for (CopyOnWriteArrayList<String> requiredClause: requiredConds) {
                        if (clause.containsAll(requiredClause)) break;
                        else{
                            count++;
                        }
                    }
    ...
    public static SimpleResponse validateActionVariableRelations(PolicyRule pr, 
        ...
        for (PolicyAction pa: pas) {
            try {
                ...
                int count = 0;
                for (CopyOnWriteArrayList<String> requiredClause: requiredConds) {
                    if (pas.containsAll(requiredClause)) break;
                    else{
                        count++;
                    }
                }
